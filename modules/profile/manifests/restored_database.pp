class profile::restored_database {
  ...
  file { '/tmp/restore_jira_postgres_backup.sh':
    ensure => present,
    owner  => 'ec2-user',
    source => 'puppet:///files/restore_jira_postgres_backup.sh'
  } ->

  exec { 'restore jira database from backup':
    command     => '/bin/bash /tmp/restore_jira_postgres_backup.sh',
    cwd         => '/tmp',
    user        => 'ec2-user',
